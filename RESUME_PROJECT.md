...
● 设计并实现 **Agent 记忆机制**：**短期记忆**通过上下文压缩组件控制对话历史的 token 预算，对旧轮次去重、摘要与截断，并在用量达到 80% 时自动触发压缩；同时将会话状态序列化持久化，支持中断恢复。**长期记忆**由大模型从对话中提取用户偏好、项目规则与关键事实，按用户级（跨项目共享）与项目级分层持久化，新会话启动时检索并注入系统提示，实现跨会话的个性化上下文。**手动压缩**通过 TUI `/compact` 触发，支持附带焦点指令保留关键上下文，并在压缩过程中顺带完成长期记忆的提取与保存。
