...
def _verify_task(task_dir: Path, repo_dir: Path, task_work_dir: Path) -> dict:
    """验证单个任务的修改."""
    model_patch_path = task_work_dir / "model.patch"
...
示例:
  ...
  python scripts/test_multi_task.py --tasks simple-calculator-bug simple-config-validation multi-method-implementation --provider kimi
    ...
    for i, task_name in enumerate(args.tasks, 1):
        ...
        print(f"\n{'='*60}")
        print("[Verification]")
        verify_result = _verify_task(task_dir, repo_dir, task_work_dir)
